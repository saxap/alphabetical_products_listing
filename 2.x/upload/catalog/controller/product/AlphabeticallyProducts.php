...
class ControllerProductAlphabeticallyProducts extends Controller{
    ...
    public function index(){
		...
		if (isset($this->request->get['char'])) {
			...
			$results = $this->model_catalog_AlphabeticallyProducts->getProducts($data_filter);
			$product_total = $this->model_catalog_AlphabeticallyProducts->getTotalProducts($data_filter); 		
			
			if (version_compare(VERSION, '2.2', '>=')) {
				$description_limit = $this->config->get($this->config->get('config_theme') . '_product_description_length'); // new
			} else {
				$description_limit = $this->config->get('config_product_description_length'); // old
			}
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('product/AlphabeticallyProducts&char='.$char, $url . '&limit=' . $value)
				);
			}
			...
			if ($page == 1) {
			    $this->document->addLink($this->url->link('product/AlphabeticallyProducts&char='.$char, true), 'canonical');
			} elseif ($page == 2) {
			    $this->document->addLink($this->url->link('product/AlphabeticallyProducts&char='.$char, true), 'prev');
			} else {
			    $this->document->addLink($this->url->link('product/AlphabeticallyProducts&char='.$char, '&page='. ($page - 1), true), 'prev');
			}

			if ($limit && ceil($product_total / $limit) > $page) {
			    $this->document->addLink($this->url->link('product/AlphabeticallyProducts&char='.$char, '&page='. ($page + 1), true), 'next');
			}
			...
			if (version_compare(VERSION, '2.2', '>=')) {
				$this->response->setOutput($this->load->view('product/AlphabeticallyProducts.tpl', $data));	
			} else {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/AlphabeticallyProducts.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/product/AlphabeticallyProducts.tpl', $data));
				} else {
					$this->response->setOutput($this->load->view('default/template/product/AlphabeticallyProducts.tpl', $data));
				}
			}

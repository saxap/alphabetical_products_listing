...
class ControllerModuleAlphabeticallyProducts extends Controller{
    ...
    public function index(){
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			$this->model_setting_setting->editSetting('AlphabeticallyProducts', $this->request->post);		
		...
		$this->data['lattxt'] = $this->language->get('lattxt');  
		$this->data['cyrtxt'] = $this->language->get('cyrtxt');  
		...
		$this->data['action'] = $this->url->link('module/AlphabeticallyProducts', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['AlphabeticallyProducts_module'])) {
			$this->data['modules'] = $this->request->post['AlphabeticallyProducts_module'];
		} elseif ($this->config->get('AlphabeticallyProducts_module')) { 
			$this->data['modules'] = $this->config->get('AlphabeticallyProducts_module');
		}		
		...
		$this->template = 'module/AlphabeticallyProducts.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);
